...
?>
				<li class="media post pingback">
					<p><?php _e( 'Pingback:', 'firstling'); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( 'Edit', 'firstling'), '<span class="edit-link">', '</span>' ); ?></p>
<?php
			break;
			default :
?>
					...
					<article id="div-comment-<?php comment_ID(); ?>" class="comment-body comment-author vcard">
						...
						<div class="media-body">
							<footer class="comment-meta">
								<h5 class="media-heading">
									<?php echo sprintf( '<strong><span class="fn">%1$s</span></strong>
														 ...
														 get_comment_author_link(), __( 'in', 'firstling'),
														 esc_url( get_comment_link( $comment->comment_ID ) ),
														 get_comment_time( 'c' ),
														 get_comment_date(), __( 'at', 'firstling'),
														 get_comment_time(), __( 'said:', 'firstling') ); ?>
								</h5>

								<?php edit_comment_link( __( 'Edit', 'firstling'), '<span class="edit-link">', ' </span>' ); ?>

								<?php if ( $comment->comment_approved == '0' ) : ?>
								<p class="comment-awaiting-moderation alert alert-info"><?php _e( 'Your comment is awaiting moderation.', 'firstling'); ?></p>
								<?php endif; ?>
							</footer><!-- .comment-meta -->
							...
							<div class="comment-metadata">
								<span class="reply-link"><?php comment_reply_link( array_merge( $args, array( 'reply_text' => __( 'Respond', 'firstling'), 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?></span>
