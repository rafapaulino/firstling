...
class TopSocialSidebar_Widget extends WP_Widget 
{
    ...
    private function create_instance($instance)
    {
        ...
        for ($a = 1; $a < 4; $a++) 
        {
            ...
            ?>
            <p>
                <label for="<?php echo esc_attr($this->get_field_id($link_href)); ?>"><?php esc_attr_e('Link','firstling'); ?>:</label>
                <input type="text" id="<?php echo esc_attr($this->get_field_id($link_href)); ?>" name="<?php echo esc_attr($this->get_field_name($link_href)); ?>" value="<?php echo esc_attr( $link ); ?>" class="widefat title" />
            </p>
            <p>
                <label for="<?php echo esc_attr($this->get_field_id($link_title)); ?>"><?php esc_attr_e('Title','firstling'); ?>:</label>
                <input type="text" id="<?php echo esc_attr($this->get_field_id($link_title)); ?>" name="<?php echo esc_attr($this->get_field_name($link_title)); ?>" value="<?php echo esc_attr( $title ); ?>" class="widefat title" />
            </p><hr>
            <?php
        ...
        for ($x = 1; $x <= count($this->icons); $x++)
        {
            ...
            <p>
                ...
                <input type="text" id="<?php echo esc_attr($this->get_field_id($link)); ?>" name="<?php echo esc_attr($this->get_field_name($link)); ?>" value="<?php echo esc_attr($link_instance); ?>" class="widefat title" />
            </p>
            <p>
                ...
                <select name="<?php echo esc_attr($this->get_field_name($icon)); ?>" id="<?php echo esc_attr($this->get_field_id($icon)); ?>" class="widefat title">
                    <?php foreach($this->icons as $key => $value): ?>
                        <option value="<?php echo esc_attr($key); ?>" <?php echo esc_attr((( $icon_instance == $key )?'selected="selected"':'')); ?>><?php echo esc_attr($value['title']); ?></option>
                    <?php endforeach; ?>
                </select>
                <hr><br>
            </p>
    ...
    private function create_html($instance)
    {
        ...
        ?>
            <li class="socialBrand">
                <a href="<?php echo esc_url($href); ?>" title="<?php echo esc_attr($title); ?>" target="_blank">
        ...
        foreach($this->icons as $key => $value)
        {
            ...
            if (trim($link_html) !== ""):
            ?>
                <li class="social <?php echo esc_attr($icon_html); ?> bgAnimated">
                    <a href="<?php echo esc_url($link_html); ?>" accesskey="<?php echo intval($x); ?>" title="<?php esc_attr_e('Click here to access:','firstling'); ?> <?php echo esc_attr($title); ?>">
                        <i class="<?php echo esc_attr($icon_class); ?>" aria-hidden="true">
                            <span class="sr-only"><?php esc_attr_e('Click here to access:','firstling'); ?> <?php echo esc_attr($title); ?></span>
                        </i>
                    </a>
                </li>               
            <?php 
