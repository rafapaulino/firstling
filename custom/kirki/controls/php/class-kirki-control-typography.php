		...
		<div class="wrapper">
			...
			<# if ( ! _.isUndefined( data.default['font-family'] ) ) { #>
				...
				<div class="font-family">
					<h5><?php esc_html_e( 'Font Family', 'firstling' ); ?></h5>
					<select {{{ data.inputAttrs }}} id="kirki-typography-font-family-{{{ data.id }}}" placeholder="<?php esc_attr_e( 'Select Font Family', 'firstling' ); ?>"></select>
				</div>
				<# if ( ! _.isUndefined( data.choices['font-backup'] ) && true === data.choices['font-backup'] ) { #>
					<div class="font-backup hide-on-standard-fonts kirki-font-backup-wrapper">
						<h5><?php esc_html_e( 'Backup Font', 'firstling' ); ?></h5>
						<select {{{ data.inputAttrs }}} id="kirki-typography-font-backup-{{{ data.id }}}" placeholder="<?php esc_attr_e( 'Select Font Family', 'firstling' ); ?>"></select>
					</div>
				<# } #>
				<# if ( true === data.show_variants || false !== data.default.variant ) { #>
					<div class="variant kirki-variant-wrapper">
						<h5><?php esc_html_e( 'Variant', 'firstling' ); ?></h5>
			...
			<# if ( ! _.isUndefined( data.default['font-size'] ) ) { #>
				...
				<div class="font-size">
					<h5><?php esc_html_e( 'Font Size', 'firstling' ); ?></h5>
			...
			<# if ( ! _.isUndefined( data.default['line-height'] ) ) { #>
				...
				<div class="line-height">
					<h5><?php esc_html_e( 'Line Height', 'firstling' ); ?></h5>
			...
			<# if ( ! _.isUndefined( data.default['letter-spacing'] ) ) { #>
				...
				<div class="letter-spacing">
					<h5><?php esc_html_e( 'Letter Spacing', 'firstling' ); ?></h5>
			...
			<# if ( ! _.isUndefined( data.default['word-spacing'] ) ) { #>
				...
				<div class="word-spacing">
					<h5><?php esc_html_e( 'Word Spacing', 'firstling' ); ?></h5>
			...
			<# if ( ! _.isUndefined( data.default['text-align'] ) ) { #>
				...
				<div class="text-align">
					<h5><?php esc_html_e( 'Text Align', 'firstling' ); ?></h5>
					<div class="text-align-choices">
						<input {{{ data.inputAttrs }}} type="radio" value="inherit" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-inherit" <# if ( data.value['text-align'] === 'inherit' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-inherit">
								...
								<span class="screen-reader-text"><?php esc_html_e( 'Inherit', 'firstling' ); ?></span>
							</label>
						</input>
						<input {{{ data.inputAttrs }}} type="radio" value="left" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-left" <# if ( data.value['text-align'] === 'left' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-left">
								...
								<span class="screen-reader-text"><?php esc_html_e( 'Left', 'firstling' ); ?></span>
							</label>
						</input>
						<input {{{ data.inputAttrs }}} type="radio" value="center" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-center" <# if ( data.value['text-align'] === 'center' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-center">
								...
								<span class="screen-reader-text"><?php esc_html_e( 'Center', 'firstling' ); ?></span>
							</label>
						</input>
						<input {{{ data.inputAttrs }}} type="radio" value="right" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-right" <# if ( data.value['text-align'] === 'right' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-right">
								...
								<span class="screen-reader-text"><?php esc_html_e( 'Right', 'firstling' ); ?></span>
							</label>
						</input>
						<input {{{ data.inputAttrs }}} type="radio" value="justify" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-justify" <# if ( data.value['text-align'] === 'justify' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-justify">
								...
								<span class="screen-reader-text"><?php esc_html_e( 'Justify', 'firstling' ); ?></span>
							</label>
						</input>
					</div>
				</div>
			...
			<# if ( ! _.isUndefined( data.default['text-transform'] ) ) { #>
				...
				<div class="text-transform">
					<h5><?php esc_html_e( 'Text Transform', 'firstling' ); ?></h5>
					<select {{{ data.inputAttrs }}} id="kirki-typography-text-transform-{{{ data.id }}}">
						...
						<option value="none"<# if ( 'none' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_html_e( 'None', 'firstling' ); ?></option>
						<option value="capitalize"<# if ( 'capitalize' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_html_e( 'Capitalize', 'firstling' ); ?></option>
						<option value="uppercase"<# if ( 'uppercase' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_html_e( 'Uppercase', 'firstling' ); ?></option>
						<option value="lowercase"<# if ( 'lowercase' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_html_e( 'Lowercase', 'firstling' ); ?></option>
						<option value="initial"<# if ( 'initial' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_html_e( 'Initial', 'firstling' ); ?></option>
						<option value="inherit"<# if ( 'inherit' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_html_e( 'Inherit', 'firstling' ); ?></option>
					</select>
				</div>
			<# } #>
			...
			<# if ( ! _.isUndefined( data.default['text-decoration'] ) ) { #>
				...
				<div class="text-decoration">
					<h5><?php esc_html_e( 'Text Decoration', 'firstling' ); ?></h5>
					<select {{{ data.inputAttrs }}} id="kirki-typography-text-decoration-{{{ data.id }}}">
						...
						<option value="none"<# if ( 'none' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_html_e( 'None', 'firstling' ); ?></option>
						<option value="underline"<# if ( 'underline' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_html_e( 'Underline', 'firstling' ); ?></option>
						<option value="overline"<# if ( 'overline' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_html_e( 'Overline', 'firstling' ); ?></option>
						<option value="line-through"<# if ( 'line-through' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_html_e( 'Line-Through', 'firstling' ); ?></option>
						<option value="initial"<# if ( 'initial' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_html_e( 'Initial', 'firstling' ); ?></option>
						<option value="inherit"<# if ( 'inherit' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_html_e( 'Inherit', 'firstling' ); ?></option>
					</select>
				</div>
			<# } #>
			...
			<# if ( ! _.isUndefined( data.default['margin-top'] ) ) { #>
				...
				<div class="margin-top">
					<h5><?php esc_html_e( 'Margin Top', 'firstling' ); ?></h5>
			...
			<# if ( ! _.isUndefined( data.default['margin-bottom'] ) ) { #>
				...
				<div class="margin-bottom">
					<h5><?php esc_html_e( 'Margin Bottom', 'firstling' ); ?></h5>
			...
			<# if ( ! _.isUndefined( data.default['color'] ) && false !== data.default['color'] ) { #>
				...
				<div class="color">
					<h5><?php esc_html_e( 'Color', 'firstling' ); ?></h5>
